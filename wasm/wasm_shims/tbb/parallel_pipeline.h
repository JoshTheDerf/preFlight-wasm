...
// Concept-y matcher so the operator& template only fires for the wrapper /
// sequence types, not arbitrary types (which would shadow std::bit_and etc).
template <typename T>
struct is_filter_like : std::false_type {};

template <typename I, typename O, typename F>
struct is_filter_like<filter_wrapper<I, O, F>> : std::true_type {};

template <typename L, typename R>
struct is_filter_like<filter_sequence<L, R>> : std::true_type {};

// ADL-friendly `&` between any two filter-like values, defined in detail so
// argument-dependent lookup at the call site finds it via filter_wrapper.
template <typename Left, typename Right,
          typename = std::enable_if_t<is_filter_like<std::decay_t<Left>>::value &&
                                      is_filter_like<std::decay_t<Right>>::value>>
...
// Placeholder `filter<I, O>` matching the oneAPI TBB shape. GCode.cpp's
// pipeline path is rewritten to a sequential loop under __EMSCRIPTEN__, so
// this type just needs to exist; it does not need to be runtime-functional.
